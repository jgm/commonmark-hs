...
htmlTagName :: Monad m => ParserT Tok s m [Tok]
htmlTagName = do
...
htmlAttributeName :: Monad m => ParserT Tok s m [Tok]
htmlAttributeName = do
  let isTagText t' = T.all isAscii t'
  let startsWithLetter t' = not (T.null t') && isLetter (T.head t')
...
htmlAttributeValueSpec :: Monad m => ParserT Tok s m [Tok]
htmlAttributeValueSpec = do
...
htmlAttributeValue :: Monad m => ParserT Tok s m [Tok]
htmlAttributeValue =
...
htmlAttribute :: Monad m => ParserT Tok s m [Tok]
htmlAttribute = do
  sps <- whitespace
  n <- htmlAttributeName
...
-- including spaces, ", ', =, <, >, or `.
htmlUnquotedAttributeValue :: Monad m => ParserT Tok s m [Tok]
htmlUnquotedAttributeValue =
  some $ noneOfToks [Spaces, LineEnd, Symbol '<', Symbol '>',
                      Symbol '=', Symbol '`', Symbol '\'', Symbol '"']

-- A single-quoted attribute value consists of ', zero or more characters
-- not including ', and a final '.
htmlSingleQuotedAttributeValue :: Monad m => ParserT Tok s m [Tok]
htmlSingleQuotedAttributeValue = do
  op <- symbol '\''
...
-- not including ", and a final ".
htmlDoubleQuotedAttributeValue :: Monad m => ParserT Tok s m [Tok]
htmlDoubleQuotedAttributeValue = do
  op <- symbol '"'
  contents <- many (satisfyTok (not . hasType (Symbol '"')))
  cl <- symbol '"'
  return $ op : contents ++ [cl]
...
-- been parsed.
htmlOpenTag :: Monad m => ParserT Tok s m [Tok]
htmlOpenTag = do
...
htmlClosingTag :: Monad m => ParserT Tok s m [Tok]
htmlClosingTag = do
  -- assume < has already been parsed
  op <- symbol '/'
  n <- htmlTagName
...
-- (See the HTML5 spec.)
htmlComment :: Monad m => ParserT Tok s m [Tok]
htmlComment = do
  -- assume < has already been parsed
  op <- sequence [ symbol '!'
                 , symbol '-'
...
htmlProcessingInstruction :: Monad m => ParserT Tok s m [Tok]
htmlProcessingInstruction = do
  -- assume < has already been parsed
  let questionmark = symbol '?'
  op <- questionmark
  contents <- many $ satisfyTok (not . hasType (Symbol '?'))
                 <|> (questionmark <* notFollowedBy (symbol '>'))
...
htmlDeclaration :: Monad m => ParserT Tok s m [Tok]
htmlDeclaration = do
  -- assume < has already been parsed
  op <- symbol '!'
...
htmlCDATASection :: Monad m => ParserT Tok s m [Tok]
htmlCDATASection = do
  -- assume < has already been parsed
  op <- sequence [ symbol '!'
                 , symbol '['
...
htmlTag :: Monad m => ParserT Tok s m [Tok]
