* TODO make attributes first-class?
support them (conditional on option) in standard parsers?
otherwise hard to see how we could add on a generic
inline attribute.
one possibility would be to make the attribute parser
configurable, defaulting to mzero.
